...
1. # 함수를 호출하는 방법
   ...
   2)메서드 호출   
   객체의 프로퍼티로 함수가 오는 경우 이를 메서드라고 합니다. 메서드를 호출할 때는 그룹 연산자인 ()를 붙여서 호출합니다.   
   ```js
      obj.m = function();  //정의
      obj.m();    //호출
   ```   
   ...
   3)생성자 호출   
   함수의 참조를 저장한 변수 앞에 new 키워드를 추가하면 함수가 생성자로 동작합니다.   
   ```js
      var obj = new Object();
   ```   
...
1. # 즉시 실행 함수
   ...
   ```js
      ...
      (function(){console.log("즉시 실행2")})();   //실행 방법1

      (function(){console.log("즉시 실행3")}());   //실행 방법2

      (function(a,b){console.log(a+b)})(4,5);      //즉시 실행 함수에 매개변수 전달

      var x = (function(){ console.log("변수 할당")})(); //변수 할당
   ```   
