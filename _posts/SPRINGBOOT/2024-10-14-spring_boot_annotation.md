...
1. # 의존성 주입이란?   
   ...
   ```   
   B클래스는 A클래스 없이 동작하지 못하므로 B클래스는 A클래스에 의존되어 있다고합니다. 이런 경우를 결합도와 의존도가 높다고 말하며 느슨한 결합을 하기 위해 어노테이션을 사용하여 빈을 생성 후 객체를 자동 주입하게 됩니다.   
   ...
   ```   
   @Component를 통해 IOC컨테이너에 A 인스턴스를 생성합니다. => A의 빈 생성   
   @Autowired를 통해 스프링 프레임워크가 자동으로 a에 A 빈, 즉 A의 인스턴스 주입하게됩니다.   

   __DI(의존성 주입)를 하게 되면 IOC(제어의 역전)가 발생합니다.__   
...
1. # 어노테이션
   ...
   __@Inject__   
   표준 DI 어노테이션, 타입을 기준으로 주입    

   __@Autowired__	   
   필드/생성자/세터 주입, 선택적 주입 가능   

   __@Resource__   
   리소스를 참조하거나 이름을 기준으로 주입   

   __@Qualifier__	  
   같은 타입의 여러 Bean 중 특정 Bean 선택   

   __@Scope__	 
   Bean의 생명주기 및 범위 설정   
 
   __@Inject vs @Autowired__   
   @Inject: 표준(Java EE, Jakarta EE) 의존성 주입 방식   
   @Autowired: Spring 전용 방식으로, Spring Context와 연계   
   Spring에서는 @Inject를 사용할 수도 있지만, Spring 특화 기능(Stereotype Annotation, Qualifier 등)을 활용하려면 @Autowired를 사용하는 것이 더 일반적입니다   
