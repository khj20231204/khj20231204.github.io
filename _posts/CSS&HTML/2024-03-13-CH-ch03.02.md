...
1. # <span id="inputButton">input으로 버튼 만들기</span>
   ...
   *input은 닫는 태그가 없기 때문에 `<input type="button">버튼을 누르세요</input>` 이런 식으로 사용하지 않음. value로 버튼에 보여지는 문자열 출력   
   ...
   ```html
      ...
      <input type="submit" value="전송"> <!-- 서버쪽으로 form 데이터를 전송 -->
      <input type="reset" value="다시 입력"> <!-- 폼에 입력된 내용을 지우고 초기화 -->
      <input type="image" src="../../imgs/CH/sunset.png" style="width:100px;height:50px"> <!-- 이미지 입력 -->
   ```   
   <input type="button" value="일반 버튼"><br>
   <input type="submit" value="전송"><br>
   <input type="reset" value="다시 입력"><br>
   <input type="image" src="../../imgs/CH/sunset.png" style="width:100px;height:50px"><br>
...
1. # <span id="button">button으로 버튼 만들기</span>
   ...
   *button태그는 닫는 태그가 있기 때문에 input 태그처럼 value 속성에 보여지는 문자열을 입력하지 않고 `<button>버튼에 보여지는 문자열</button>`이런 식으로 닫는 태그 안에 버튼에 보여지는 문자열을 입력합니다.   
   ...
   ```html
      ...
      <button type="submit">전송</button> <!-- 서버쪽으로 form 데이터를 전송 -->
      <button type="reset">다시 입력</button> <!-- 폼에 입력된 내용을 지우고 초기화 -->
      <button type="button"><img src="../../imgs/CH/sunset.png" style="width:100px;height:50px"></button> <!-- 이미지 입력 -->
   ```   
   <button type="button">일반 버튼</button>   
   <br>
   ...
   <br>
   <button type="button"><img src="../../imgs/CH/sunset.png" style="width:100px;height:50px"></button>
...
1. # <span id="checkradio">check박스와 radio박스</span>
   ...
   *checked 이 속성이 있으면 초기에 선택 상태가 됩니다.   
   ...
   ```html
      <input type="checkbox" value="math">수학
      <input type="checkbox" value="eng">영어
      <input type="checkbox" value="physics">물리
   ```   
   <input type="checkbox" value="math" checked>수학
   <input type="checkbox" value="eng">영어
   <input type="checkbox" value="physics">물리
   <br>   
   
   다중 선택이 가능하기 때문에 그룹을 묶을 필요가 없이 개별값을 서버에 넘겨주게 됩니다.   
...
1. # <span id="select">select</span>
   ...
   *size는 최초 보이는 항목의 갯수로 디폴트는 1입니다.   
   ...
   ```   
    <select>
      <option value="kor" selected>한국어
      <option value="eng">영어
      <option value="china">중국어   
   </select>

   서버에서 데이터를 받을 땐 value 값들인 kor, eng, china를 받게 됩니다.   
