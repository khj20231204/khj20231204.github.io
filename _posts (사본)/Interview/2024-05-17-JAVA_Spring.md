...
1. # MVC
   유연성(기능의 확장, 교체, 변경),유지보수를 편하게 하기 위해 자바가 객체 간의 밀집도, 결합도, 의전도를 떨어뜨리기 위해 정보은닉이란 개념이 나왔다면 스프링은 기능 간의 밀집도, 결합도, 의존도를 떨어뜨리기 위해서 나온 방법이 기능을 분리하자는 것이고 이것을 실현한 것이 MVC패턴 입니다. 기존 MVC1에서 jsp가 모든 것을 처리 했지만 MVC2 패턴에서는 Model, View, Controller로 기능을 나누어 구현하게 됩니다.   
   Model : 사용자가 편집하길 원하는 모든 데이터를 가지고 있어야 한다. 애플리케이션의 정보, 데이타를 나타냅니다. 데이타베이스, 처음의 정의하는 상수, 초기화값, 변수 등을 뜻합니다. 또한 이러한 DATA, 정보들의 가공을 책임지는 컴포넌트를 말합니다.   
   View :  사용자 인터페이스 요소를 나타냅니다. 다시 말해 데이터 및 객체의 입력, 그리고 보여주는 출력을 담당합니다. 데이타를 기반으로 사용자들이 볼 수 있는 화면입니다.  
   Controller : 데이터와 사용자인터페이스 요소들을 잇는 다리역할을 합니다. 서버와 클라이언트이 요구에 적절하게 Model과 View를 선택하는 역할을 합니다. 
   Model과 View는 서로에 대해서 몰라야 하고, Controller는 Model과 View에 대해서 알아야 합니다.   
...
1. # 스프링 컨텍스트
   ...
   서블릿 각자의 Bean들이 모인 공간. 웹, 앱 마다 한 개씩 존재하므로 웹 앱 그 자체를 의미하기도 함. 이 Context 내에서의 Bean들은 서로 공유될 수 없음.  
